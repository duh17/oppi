...
async function cmdEnv(action: string | undefined): Promise<void> {
  const envPath = join(homedir(), ".config", "oppi", "env");

  switch (action) {
    case "init": {
      // Snapshot current shell's PATH into env file
      const currentPath = process.env.PATH || "";
      if (!currentPath) {
        console.log(chalk.red("  ✗ $PATH is empty — run this from your interactive shell"));
        process.exit(1);
      }
      ...
      const lines = [
        "# oppi-server host environment",
        "# Generated by: oppi-server env init",
      ...
      console.log(chalk.dim(`    ${dedupedEntries.length} PATH entries captured`));
      console.log(chalk.dim("    Restart oppi-server to apply."));
      break;
    }
    ...
    case "show": {
      ...
      if (nonPath.length > 0) {
        console.log("");
        console.log(chalk.bold("  Other overrides:"));
