...
Iroh-only credentials fail closed. For Iroh-preferred credentials, a verified HTTPS Bonjour LAN endpoint wins locally. Self-signed endpoints require the paired leaf pin; that pin is authoritative when present. Tailscale hostnames without a stable pin may instead use system public-CA hostname validation on the exact paired port because their public leaf certificates rotate. Plaintext LAN candidates are rejected. Otherwise the client prefers Iroh and may use the signed HTTP endpoint (normally Tailscale) only when an Iroh reachability probe fails before application traffic starts or at an explicit network/foreground recovery boundary. The HTTP fallback stays sticky between boundaries. Malformed metadata, peer mismatch, authentication failure, ALPN negotiation failure, framing failure, and protocol failure never downgrade; a terminal Iroh failure invalidates any previously usable fallback. An unhealthy established Iroh route replaces that server's complete transport composition immediately instead of redialing through the same cached QUIC connection: persistent streams and the manager close, route selection and URL-bound clients rebuild, subscription intent is restored, and server lists force-refresh. Automatic recovery has a five-attempt budget: the first full rebuild is immediate, subsequent attempts use exponential 1-, 2-, 4-, and 8-second spacing, and no sixth automatic rebuild runs. A live stream reconnect restores the budget; explicit Retry Connection bypasses both cooldown and exhaustion. Explicit reconfiguration and persistent transport replacement advance a sender generation, including LAN/paired handoffs and a post-delay retry check, so an in-flight mutation cannot retry across lanes. Superseded setup attempts clean up only managers that no newer setup adopted. Short-lived clients perform a bounded Bonjour probe before transport selection, probe Iroh before invoking their operation when LAN is absent, and never replay a started operation over HTTP. Loopback URLs are runtime-only and never become the paired server identity.
