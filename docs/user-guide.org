...
to dispatch a short prompt. It can be triggered from a Control Center
widget or Siri shortcut.
...
floating pill appears at the bottom of the timeline showing the tool
name and a summary of the action. Tap Allow or Deny directly on the
pill for quick decisions.

For more detail, tap the pill to open the permission sheet. The sheet
shows the full command or file path, the agent's reason for the
request, and scope options:

| Choice        | Effect                                     |
|---------------+--------------------------------------------|
| Allow once    | Permits this single tool call              |
| This session  | Permits matching calls for this session    |
| Always        | Creates a persistent rule for all sessions |
| Deny once     | Blocks this single tool call               |
| Deny globally | Blocks all matching calls going forward    |
...
C, C++, Shell, SQL, HTML, CSS, JSON, YAML, TOML, Markdown,
Org, LaTeX, Diff, Zig, Protobuf, GraphQL, Ruby
#+end_src
...
- *Double-tap* a code block in the timeline to copy its contents.
...
.c .cpp .cc .cxx .h .hpp .hxx .rb .sh .bash .zsh .fish .sql
.graphql .gql .proto .zig .html .css .json .yml .yaml .toml .md .org
.tex .latex .diff
#+end_src
...
These files without extensions are recognized:
