...
The network path, DNS resolver, and any TLS terminator you configure can observe ordinary connection metadata: IP addresses, hostnames, SNI, timing, duration, and approximate traffic volume. Pairing invites, access tokens, and HTTP/WebSocket requests remain subject to the configured transport and authentication settings. See [SECURITY.md](../SECURITY.md) and [Networking](networking.md).
...
See the detailed [telemetry policy](telemetry.md).
