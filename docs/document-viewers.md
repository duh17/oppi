...
An expanded, completed, successful text `write` row opens the current file directly on this navigation stack. Workspace sessions fetch it through the source session route, which preserves the server, workspace, worktree or sandbox mapping, and exact structured path. Guest and worktree-relative children followed from that reader keep the same immutable session origin and session-raw route. A wiki link classified as a host file opens authenticated GET `/files/raw` from a session-origin reader only when workspace runtime is confirmed host; sandbox and unknown runtime keep session-raw even for that classification. Workspace-less control sessions use their declared source-server host route, with relative paths resolved from the server-owned control-session working directory. Oppi does not use the recorded write content or retry another origin when the current file is unavailable. Empty writes retain an expanded “Open current file” surface with the same double-tap, pinch, context-menu, and accessibility activation. Running, failed, and interrupted writes keep the tool-output viewer. Reads, edit diffs, generic extension output, terminal output, and stored media keep their existing viewer behavior.
...
The workspace file browser, fuzzy `/paths` index, and workspace `contents`/`raw` routes keep the requested path lexically inside the workspace root, then follow in-tree symlink names to their real path even when the target is outside the workspace. Lexical `..` traversal and absolute escapes are still rejected. Search does not follow symlink aliases for indexing. Host-workspace session-raw is not workspace-confined: an owner-authenticated session may read an explicit real path that exists, including in-tree symlink targets and absolute/~ host paths. Pairing/auth is the gate. Sandbox session-raw still 403s outside and guest-escape paths.

Owner host-file reads use a separate authenticated GET/HEAD `/files/raw?path=` helper. That route accepts absolute paths and expands bare `~` / `~/`. A relative path requires `controlSessionId` naming an existing declared control session; it resolves against the same server-owned working directory used by that session's runtime. Unknown or workspace-owned session IDs do not authorize relative reads. The route realpaths a regular file, returns the canonical path as percent-encoded `X-Oppi-Resolved-Path`, and never lists directories. Current control-file readers use the canonical path returned with the bytes as the base for source-relative Markdown links. Wiki-link parsing accepts local `file://` URLs and rejects undocumented `file:/...`. It does not apply workspace-root confinement or a secret-name 403. Pairing/auth is the remote gate. Host HTML and SVG still use the existing fetch → `loadHTMLString` + CSP viewer; WKWebView must not URL-load `/files/raw`.

Directory listings, fuzzy `/paths`, and workspace `raw` can show owner-visible names including `.env`, keys, credential files, and files that are not under ignored VCS/build directories. Workspace listings and `contents`/`raw` stay lexically inside the workspace and may follow in-tree symlinks. Host `/files/raw` and host-workspace session-raw stay owner-authenticated. Sandbox session-raw stays confined. Agents must not ingest secrets.
