...
An expanded, completed, successful text `write` row opens the current file directly on this navigation stack. Workspace sessions fetch it through the source session route, which preserves the server, workspace, worktree or sandbox mapping, and exact structured path; links followed from that reader retain the same immutable session origin and never retry main or owner-host lookup. Workspace-less control sessions use their declared source-server host route, with relative paths resolved from the server-owned control-session working directory. Oppi does not use the recorded write content or retry another origin when the current file is unavailable. Empty writes retain an expanded “Open current file” surface with the same double-tap, pinch, context-menu, and accessibility activation. Running, failed, and interrupted writes keep the tool-output viewer. Reads, edit diffs, generic extension output, terminal output, and stored media keep their existing viewer behavior.
...
Directory listings, fuzzy `/paths`, and workspace `raw` can show owner-visible names including `.env`, keys, credential files, and files that are not under ignored VCS/build directories. Workspace routes stay `realpath`-confined to the workspace. Host `/files/raw` stays owner-authenticated. Agents must not ingest secrets.
