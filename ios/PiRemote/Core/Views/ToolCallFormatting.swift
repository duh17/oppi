...
enum ToolCallFormatting {
    ...
    /// Returns nil for unknown payloads so callers can fall back to raw output.
    ...
    // MARK: - Todo Output Helpers

    private static let todoListMaxItemsPerSection = 12

    private static func todoAction(args: [String: JSONValue]?, argsSummary: String) -> String? {
    ...
    private static func todoAppendDiffLines(
        ...
        let lines = todoBodyLines(from: trimmed)
        return lines.map { DiffLine(kind: .added, text: $0) }
    }
                ...
                if bodyLines.count == 1 {
                    ...
                    if bodyLines.count > todoUpdateBodyPreviewLineLimit {
                        lines.append(
                            DiffLine(
                                kind: .added,
                                text: "  … +\(bodyLines.count - todoUpdateBodyPreviewLineLimit) more lines"
                            )
                        )
                    }
                }
    ...
    private static func todoDiffPreview(
    ...
    private static func formattedTodoListLine(_ item: TodoItemPayload) -> String {
        ...
        if let status = normalizedTodoStatus(item.status) {
            parts.append("`\(status)`")
        }

        if let title = compactTodoText(item.title, max: 140) {
            parts.append(title)
        }
    ...
    private static func formattedTodoItemMarkdown(_ item: TodoItemPayload, action: String?) -> String {
        ...
        if let status = normalizedTodoStatus(item.status) {
            headerParts.append("`\(status)`")
        }

        if let created = compactTodoText(item.createdAt, max: 80) {
            headerParts.append(created)
        }
    ...
    private static func normalizedTodoStatus(_ raw: String?) -> String? {
        guard let raw else { return nil }
    ...
    private struct TodoItemPayload: Decodable, Sendable {
        let id: String?
        let title: String?
        let tags: [String]?
        let status: String?
    ...
    private struct TodoListPayload: Decodable, Sendable {
