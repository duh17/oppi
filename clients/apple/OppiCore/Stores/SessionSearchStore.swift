...
private let logger = Logger(
    ...
    category: "SessionSearchStore"
)

/// Platform HTTP client that can run session FTS.
///
/// iOS `APIClient` and Mac `MacWorkspaceClient` both conform. The store stays
/// UI-framework-free so Home and inbox can share one search policy.
...
final class SessionSearchStore {
    ...
    ///
    /// Emphasis uses `inlinePresentationIntent` only. Platform painters apply
    /// highlight color.
    ...
    func search(query: String, workspaceId: String? = nil, apiClient: (any SessionSearching)?) {
        searchTask?.cancel()
