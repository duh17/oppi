...
/// Shared cell generation for the session-id π-grid badge.
///
/// iOS and Mac paint this result in their own layers. No UI framework imports.
enum SessionGridRenderer {

    // MARK: - Types

    enum CellRole: Equatable, Sendable {
        case scatter, growth, piExposed, piEdge, piCore, almostSpark, spark
    }

    struct CellData: Equatable, Sendable {
        let row: Int
        let col: Int
        let role: CellRole
