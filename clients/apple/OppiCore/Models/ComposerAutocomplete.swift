...
enum ComposerAutocomplete {
    ...
    /// Keep `/compact` available even before `get_commands` returns, matching iOS.
    static func availableCommands(from serverCommands: [SlashCommand]) -> [SlashCommand] {
    ...
    static func insertFileMention(path: String, isDirectory: Bool, into text: String) -> String {
        guard let tokenRange = activeTokenRange(in: text) else {
            return text
        }
        ...
        updated.replaceSubrange(tokenRange, with: "@\(path)\(suffix)")
        return updated
    }
