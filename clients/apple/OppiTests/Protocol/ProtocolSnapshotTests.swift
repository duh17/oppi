...
///
/// Decodes canonical ServerMessage JSON maintained by the explicit updater
/// (`cd server && npm run protocol:fixtures:update`). If any message fails to
/// decode, the server and iOS are out of sync.
///
/// To regenerate the snapshot files explicitly:
///   cd server && npm run protocol:fixtures:update
...
struct ProtocolSnapshotTests {
    ...
    @Test func snapshotFileExists() throws {
        #expect(
            ...
            "Protocol snapshot not found at \(snapshotURL.path). Run: cd server && npm run protocol:fixtures:update"
        )
    }
