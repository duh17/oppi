...
struct ConnectionCoordinatorTests {
    ...
    @Test func activeAudioPlaybackPresentationPrefersActiveServer() {
        let (coordinator, _) = makeCoordinator()
        let serverA = makeServer(id: "sha256:audio-a", name: "A")
        let serverB = makeServer(id: "sha256:audio-b", name: "B")

        coordinator.serverStore.addOrUpdate(serverA)
        coordinator.serverStore.addOrUpdate(serverB)

        coordinator.switchToServer(serverA)
        let connectionA = coordinator.activeConnection
        connectionA.audioPlayer.setSessionContext(
            makeTestSession(id: "s-a", name: "Session A", model: "anthropic/claude-sonnet-4")
        )
        connectionA.audioPlayer._setPlaybackStateForTesting(playing: "voice-a", loading: nil)

        coordinator.switchToServer(serverB)
        let connectionB = coordinator.activeConnection
        connectionB.audioPlayer.setSessionContext(
            makeTestSession(id: "s-b", name: "Session B", model: "openai/o4-mini")
        )
        connectionB.audioPlayer._setPlaybackStateForTesting(playing: "voice-b", loading: nil)

        let presentation = coordinator.activeAudioPlaybackPresentation
        #expect(presentation?.sessionID == "s-b")
        #expect(presentation?.title == "Session B")
        #expect(presentation?.subtitle == "o4-mini")
        #expect(presentation?.provider == "openai")
    }

    @Test func activeAudioPlaybackPresentationFallsBackToBackgroundServer() {
        let (coordinator, _) = makeCoordinator()
        let serverA = makeServer(id: "sha256:audio-fallback-a", name: "A")
        let serverB = makeServer(id: "sha256:audio-fallback-b", name: "B")

        coordinator.serverStore.addOrUpdate(serverA)
        coordinator.serverStore.addOrUpdate(serverB)

        coordinator.switchToServer(serverA)
        let backgroundConnection = coordinator.activeConnection
        backgroundConnection.audioPlayer.setSessionContext(
            makeTestSession(id: "s-a", name: "Session A", model: "google/gemini-2.5-pro")
        )
        backgroundConnection.audioPlayer._setPlaybackStateForTesting(playing: "voice-a", loading: nil)

        coordinator.switchToServer(serverB)

        let presentation = coordinator.activeAudioPlaybackPresentation
        #expect(presentation?.sessionID == "s-a")
        #expect(presentation?.title == "Session A")
        #expect(presentation?.subtitle == "2.5-pro")
        #expect(presentation?.provider == "google")
    }

    @Test func activeAudioPlaybackPresentationStaysBoundToPlaybackSession() {
        let (coordinator, _) = makeCoordinator()
        let server = makeServer(id: "sha256:audio-rebind", name: "A")

        coordinator.serverStore.addOrUpdate(server)
        coordinator.switchToServer(server)

        let connection = coordinator.activeConnection
        connection.audioPlayer.setSessionContext(
            makeTestSession(id: "s-a", name: "Session A", model: "openai/o4-mini")
        )
        connection.audioPlayer._setPlaybackStateForTesting(playing: "voice-a", loading: nil)

        connection.audioPlayer.setSessionContext(
            makeTestSession(id: "s-b", name: "Session B", model: "anthropic/claude-sonnet-4")
        )
