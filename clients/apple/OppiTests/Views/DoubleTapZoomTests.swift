...
struct DoubleTapZoomTests {
    ...
    @Test func zoomableGraphicalViewHasNoSegmentedControl() throws {
        ...
        body.frame = CGRect(x: 0, y: 0, width: 390, height: 700)
        body.layoutIfNeeded()
        let diagram = try #require(timelineFirstView(ofType: ZoomableGraphicalView.self, in: body))
        #expect(timelineAllViews(in: diagram).contains { $0 is UISegmentedControl } == false)
        #expect(diagram.debugDoubleTapRecognizerCountForTesting == 1)
    }
