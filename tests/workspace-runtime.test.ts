...
function id(userId: string, workspaceId: string, sessionId: string): WorkspaceSessionIdentity {
...
describe("WorkspaceRuntime", () => {
  ...
  describe("slot tracking", () => {
    it("reserves and counts a session slot", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      expect(rt.getWorkspaceSessionCount(U, W)).toBe(1);
      expect(rt.globalSessionCount).toBe(1);
    });

    it("tracks multiple sessions in one workspace", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      rt.reserveSessionStart(id(U, W, "s2"));
      expect(rt.getWorkspaceSessionCount(U, W)).toBe(2);
      expect(rt.globalSessionCount).toBe(2);
    });

    it("tracks sessions across workspaces", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      rt.reserveSessionStart(id(U, W2, "s2"));
      expect(rt.getWorkspaceSessionCount(U, W)).toBe(1);
      expect(rt.getWorkspaceSessionCount(U, W2)).toBe(1);
      expect(rt.globalSessionCount).toBe(2);
    });

    it("releases a session slot", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      rt.releaseSession(id(U, W, "s1"));
      expect(rt.getWorkspaceSessionCount(U, W)).toBe(0);
    ...
    it("release is idempotent", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      rt.releaseSession(id(U, W, "s1"));
      rt.releaseSession(id(U, W, "s1")); // no throw
      expect(rt.getWorkspaceSessionCount(U, W)).toBe(0);
    });

    it("release of unknown session is safe", () => {
      rt.releaseSession(id(U, W, "nonexistent")); // no throw
    });

    it("markSessionReady is a noop (no error)", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      rt.markSessionReady(id(U, W, "s1")); // no throw
    });

    it("rejects duplicate reservation", () => {
      rt.reserveSessionStart(id(U, W, "s1"));
      expect(() => rt.reserveSessionStart(id(U, W, "s1"))).toThrow("already reserved");
    });
  });
  ...
  describe("session limits", () => {
    it("enforces per-workspace limit", () => {
      ...
      expect(() => rt.reserveSessionStart(id(U, W, "s3"))).toThrow("Workspace session limit");
      // Different workspace is fine
      rt.reserveSessionStart(id(U, W2, "s3"));
    });
    ...
    it("enforces global limit", () => {
      ...
      expect(() => rt.reserveSessionStart(id(U, W, "s3"))).toThrow("Global session limit");
    });
    ...
    it("workspace limit checked before global limit", () => {
      ...
      const err = (() => {
        try { rt.reserveSessionStart(id(U, W, "s2")); }
        catch (e) { return e; }
      })();
  ...
  describe("locks", () => {
    it("withSessionLock serializes same-session operations", async () => {
      ...
      const p1 = rt.withSessionLock(U, "s1", async () => {
        await new Promise((r) => setTimeout(r, 10));
        order.push(1);
      });

      const p2 = rt.withSessionLock(U, "s1", async () => {
        order.push(2);
      });
    ...
    it("withSessionLock allows parallel for different sessions", async () => {
      ...
      const p2 = rt.withSessionLock(U, "s2", async () => {
        order.push(2);
      });
    ...
    it("withWorkspaceLock serializes same-workspace operations", async () => {
      ...
      const p1 = rt.withWorkspaceLock(U, W, async () => {
        await new Promise((r) => setTimeout(r, 10));
        order.push(1);
      });

      const p2 = rt.withWorkspaceLock(U, W, async () => {
        order.push(2);
      });
    ...
    it("withWorkspaceLock allows parallel for different workspaces", async () => {
      ...
      const p2 = rt.withWorkspaceLock(U, W2, async () => {
        order.push(2);
      });
    ...
    it("lock is released on error", async () => {
      await expect(
        rt.withSessionLock(U, "s1", async () => { throw new Error("oops"); }),
      ).rejects.toThrow("oops");

      // Lock released — second call should succeed immediately
      const result = await rt.withSessionLock(U, "s1", async () => "ok");
  ...
  describe("queries", () => {
    it("getWorkspaceSessionCount returns 0 for unknown workspace", () => {
      expect(rt.getWorkspaceSessionCount(U, "unknown")).toBe(0);
    });
