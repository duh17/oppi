...
Scope: `pi-remote` permission gate + policy engine + iOS approval UX

Related technical docs:
- `pi-remote/docs/policy-engine-v2.md` (full architecture/data model)
- `pi-remote/src/policy.ts` (evaluation logic)
- `pi-remote/src/gate.ts` (approval lifecycle + fail-safe)
- `pi-remote/src/rules.ts` (learned/manual rule store)
- `pi-remote/src/audit.ts` (audit log)
