...
"Allow always" for an executable = add a learned rule to `~/.config/pi-remote/rules.json`.
...
Storage: `~/.config/pi-remote/audit.jsonl` — append-only, one JSON object per line. Rotate at 10MB.
