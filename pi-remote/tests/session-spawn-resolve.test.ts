...
describe("session-spawn resolvePiExecutable", () => {
  const originalEnv = process.env.PI_REMOTE_PI_BIN;
  ...
  afterEach(() => {
    if (originalEnv === undefined) {
      delete process.env.PI_REMOTE_PI_BIN;
    } else {
      process.env.PI_REMOTE_PI_BIN = originalEnv;
    }
  });

  it("prefers PI_REMOTE_PI_BIN when it exists", () => {
    process.env.PI_REMOTE_PI_BIN = "/custom/pi";
