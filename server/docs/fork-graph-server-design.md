...
```json
{
  ...
  "parentSession": "/Users/dev/.pi/agent/sessions/.../2026-02-11T19-59-35-161Z_79a0a8db-8a00-4354-9c45-d1a05338c5ba.jsonl"
}
```
...
interface ForkGraphNode {
  ...
  // projection metadata
  workspaceId: string;
  attachedSessionIds: string[]; // oppi-server sessions that reference this file/id
...
interface ForkGraph {
  ...
  current?: {
    sessionId: string;   // oppi-server session id
...
Notes:
- Node identity is `piSessionId`, not oppi-server `Session.id`.
- Multiple oppi-server sessions may point to one node over time.
...
For one workspace:
1. all persisted oppi-server sessions in that workspace
...
Optional query:
- `sessionId=<sid>` → populate `current` anchor (oppi-server session id)
