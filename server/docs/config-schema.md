...
| `pairHost`      | string | *(unset)*          | Last explicit pairing advertise host from `oppi pair --host` / `oppi serve --host`. Hostname or IP only (no scheme, no port). Not the bind address. Doctor and later pair commands read this after `--host` / `OPPI_PAIR_HOST`. |
