...
Same model for oppi-server containers.
...
3. Callback returns to oppi-server server
...
│    credential.helper │    │  ~/.config/oppi-server/           │
...
```json
// ~/.config/oppi-server/integrations.json (encrypted at rest)
{
  "github": {
    "type": "oauth",
...
redirects to a custom URL scheme (oppi-server://) that closes the browser
