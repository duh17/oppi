...
For agents, prefer the `build_theme` tool from the `oppi-admin` workspace extension when it is enabled. It validates the 49-token color map and writes the theme to the server's configured theme directory.
