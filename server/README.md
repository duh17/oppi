...
oppi session ...             # create/send/wait/inspect/resume/fork/delete sessions
...
Saved Agents store reusable user definitions. An optional `icon` string accepts one Unicode emoji or an SF Symbol name; `icon: null` clears it. Saved-Agent launches snapshot the current Agent version and icon into session launch metadata, so later Agent edits do not change existing session rows or chat identity. Unsupported SF Symbols and malformed persisted values render the generic Agent icon without affecting execution. Pi Control sessions have no saved Agent identity. They load ordinary global Pi configuration, including `~/.pi/agent/SYSTEM.md`, `APPEND_SYSTEM.md`, settings, tools, Skills, and Extensions. They run with host-user permissions and can use the installed `oppi` CLI through normal `bash`. Oppi does not register a separate managed `oppi` or `ask` tool.
