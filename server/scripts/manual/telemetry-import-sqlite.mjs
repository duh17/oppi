...
function listTelemetryFiles(telemetryDir) {
  return readdirSync(telemetryDir)
    .filter((name) => {
      ...
      return name.startsWith(CHAT_PREFIX) || name.startsWith(METRICKIT_PREFIX);
    })
    .sort((a, b) => a.localeCompare(b))
    .map((name) => ({
      name,
      path: join(telemetryDir, name),
      kind: name.startsWith(CHAT_PREFIX) ? "chat" : "metrickit",
    }));
}
  ...
  const runImport = () => {
    ...
    console.log(
      `[telemetry-import] files=${result.filesSeen} imported=${result.filesImported} `
        ...
        + `metrickitPayloads=${result.metrickitPayloadsImported} parseErrors=${result.parseErrors} `
