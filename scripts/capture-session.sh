...
DEVICE_QUERY=""
LAST="15m"
OUTPUT_DIR="$HOME/Library/Logs/PiRemote/captures"
INCLUDE_DEBUG=0
INCLUDE_PERF=0
INCLUDE_LIFECYCLE=0
KEEP_ARCHIVE=0
...
usage() {
  cat <<'EOF'
Capture focused PiRemote iOS logs for a session with minimal noise.
...
Options:
  ...
  -d, --device <id|name>     Device selector for log collection.
      ...
      --output-dir <path>    Output directory (default: ~/Library/Logs/PiRemote/captures)
...
BASE_PREDICATE='subsystem == "dev.chenda.PiRemote" OR process == "PiRemote"'
...
CORE_PATTERN="\\[dev\\.chenda\\.PiRemote:(${CORE_CATEGORIES})\\]"
...
source_subsystem: dev.chenda.PiRemote
source_process: PiRemote
raw_log: raw-app.log
