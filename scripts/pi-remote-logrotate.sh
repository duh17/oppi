...
LOG_FILE="${PI_REMOTE_LOG_FILE:-$HOME/.local/var/log/pi-remote.log}"
...
usage() {
  cat <<'EOF'
Rotate pi-remote log file with copy+truncate semantics (launchd-friendly).

Usage:
  scripts/pi-remote-logrotate.sh [options]

Options:
      --file <path>     Log file to rotate (default: ~/.local/var/log/pi-remote.log)
