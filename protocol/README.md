...
| `pi-events.json` | All 34 pi SDK `AgentSessionEvent` shapes | Hand-maintained reference (used to test `translatePiEvent` in `session-protocol.ts`) |
...
| `/app/events/stream` | Server-to-client JSON | `AppEventStreamMux` | app event stream coordinator | Global row, attention, extension UI, and workspace invalidation events. Reconnect catch-up comes from HTTP snapshots. |
...
| `/dictation/stream` | Bidirectional JSON + binary | `DictationStreamMux` | `DictationStreamClient` | Server dictation control messages and PCM audio frames. |
